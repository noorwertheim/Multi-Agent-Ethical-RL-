''' IDQN algorithm for external value embedding. Compatible with lumberjacks_external.py'''
...
class ReplayBuffer:
    ...
    def sample(self, n):
        ...
        for transition in mini_batch:
            s, a, r, s_prime, done, ethical, total = transition
            s_lst.append(s)
            a_lst.append(a)
            r_lst.append(r)
...
def test(env, num_episodes, q):
    ...
    ethical_score = np.zeros(env.n_agents)
    for episode_i in range(num_episodes):
        state = env.reset()
        done = [False for _ in range(env.n_agents)]
        while not all(done):
            action = q.sample_action(torch.Tensor(state).unsqueeze(0), epsilon=0)[0].data.cpu().numpy().tolist()
            next_state, reward, done, info, ethical_reward = env.step(action)
            score += np.array(reward)
            ethical_score += np.array(ethical_reward)
            state = next_state
    return sum(score / num_episodes), sum(ethical_score / num_episodes)
...
def main(env_name, lr, gamma, batch_size, buffer_limit, log_interval, max_episodes,
    ...
    score = np.zeros(env.n_agents)
    ethical_score = np.zeros(env.n_agents)
    total_score = np.zeros(env.n_agents)
    for episode_i in range(max_episodes):
        ...
        while not all(done):
            ...
            next_state, reward, done, info, ethical_reward, total_reward = env.step(action)
            memory.put((state, action, (np.array(reward)).tolist(), next_state, np.array(done, dtype=int).tolist(), (np.array(ethical_reward)).tolist()))
            score += np.array(reward)
            ethical_score += np.array(ethical_reward)
            total_score += np.array(total_reward)
            state = next_state
        ...
        if episode_i % log_interval == 0 and episode_i != 0:
            ...
            ethical_test_score = test(test_env, test_episodes, q)[1]
            print("#{:<10}/{} episodes , avg train score : {:.1f}, test score: {:.1f} n_buffer : {}, eps : {:.1f}"
                  .format(episode_i, max_episodes, sum(score / log_interval), test_score, memory.size(), epsilon))
            print('ethical score', sum(ethical_score / log_interval), 'ethical test score', ethical_test_score, 'total score', total_score)
            if USE_WANDB:
                wandb.log({'episode': episode_i, 'test-score': test_score, 'ethical test score': ethical_test_score,
                           'buffer-size': memory.size(), 'epsilon': epsilon, 'train-score': sum(score / log_interval), 'ethical train score': sum(ethical_score / log_interval)})
            score = np.zeros(env.n_agents)
            ethical_score = np.zeros(env.n_agents)
            total_score = np.zeros(env.n_agents)
