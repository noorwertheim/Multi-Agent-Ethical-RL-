''' External embedding of ethical values: 
Avoid being greedy by implementing that if an agent has more trees than another agent, the reward for cutting a tree will be lower. 
This version of lumberjacks contains 2 agents whom operate according to the collective value.
I.e. when the other agent has less trees, the agent will receive lower reward for cutting trees, which will train agent to cut less trees, hopefully.
In reward function --> consequentialist approach.

agent 0 = weak agent
--> he will only cut weak_percentage % of the trees he encounters
agent 1 = strong agent
--> is able to cut all trees he encounters
'''
...
class Lumberjacks(gym.Env):
    ...
    def __init__(self, grid_shape: Coordinates = (5, 5), n_agents: int = 2, n_trees: int = 12,
        ...
        self._lower_cutdown_reward = lower_cutdown_reward
    ...
    def step(self, agents_action: List[int]):
        ...
        if True in mask: 
            ...
            # if the agent is the weak agent: 
            if cut_agent == 0: 
                # he will only cut the tree in weak_percentage % of the cases
            ...
            # strong agent will always cut the tree
            else: 
                # External ethical embedding: 
                # If the other agent does not have less trees than the cutting agent, he will get the full cutting reward. 
                if self._total_trees_cut[cut_agent] != max(self._total_trees_cut) or self._total_trees_cut[0] == self._total_trees_cut[1]:
                    self._total_trees_cut[cut_agent] += 1
                    rewards[cut_agent] += self._tree_cutdown_reward
                    self._tree_map[mask] = 0
