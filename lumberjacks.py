''' Lumberjacks version with ethics, and weak and strong agent, but separate ethical weights. 
...
class Lumberjacks(gym.Env):
    ...
    def __init__(self, grid_shape: Coordinates = (5, 5), n_agents: int = 2, n_trees: int = 18,
                 ...
                 step_cost: float = -1, tree_cutdown_reward: float = 10, max_steps: int = 150, weak_percentage: float = 0.5, normative_reward: float = -10, evaluative_reward: float = 10, w0: float = 1, we1: float = 0.7, we2: float = 0.5):
        assert 0 < n_agents
        ...
        self._normative_reward = normative_reward
        self._evaluative_reward = evaluative_reward
        self._w0 = w0
        self._we1 = we1
        self._we2 = we2
    ...
    def step(self, agents_action: List[int]):
        ...
        if True in mask: 
            ...
            if count > 1:
                ...
                if random.random() <= self._weak_percentage:
                    ...
                    elif self._total_trees_cut[0] == 0 and (self._total_trees_cut[1] > 3):
                        ethical_rewards[1] += self._normative_reward
                ...
                elif self._total_trees_cut[1] == 0 and (self._total_trees_cut[0] > 3):
                    ethical_rewards[0] += self._normative_reward
            ...
            # weak agent will only cut in weak_percentage % of the cases
                if cut_agent == 1: 
                    if random.random() <= self._weak_percentage: 
            ...
            # strong agent can always cut
                    ...
                    self._total_trees_cut[cut_agent]
                    rewards[cut_agent] += self._tree_cutdown_reward
                    if self._total_trees_cut[1] and self._total_trees_cut[0] > 0:
                        if self._total_trees_cut[1] / self._total_trees_cut[0] < 0.5: 
                            ethical_rewards[0] += self._normative_reward
        ...
        return self.get_agent_obs(), total_rewards, self._agent_dones, {}, ethical_rewards, rewards
