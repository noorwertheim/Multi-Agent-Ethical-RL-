...
def test(env, num_episodes, q):
    ...
    return score / num_episodes, ethical_score / num_episodes, individual_score / num_episodes
        ...
        if (episode_i + 1) % log_interval == 0:
            ...
            if USE_WANDB:
                wandb.log({'episode': episode_i, 'test-score': test_score, 'ethical-test-score': test_ethical, 'individual-test-score': test_individual, 'buffer-size': memory.size(),
                           'epsilon': epsilon, 'train-score': train_score, 'ethical-train-score': train_ethical, 'individual-train-score': train_individual})
            score = 0
            ethical_score = 0
            individual_score = 0
